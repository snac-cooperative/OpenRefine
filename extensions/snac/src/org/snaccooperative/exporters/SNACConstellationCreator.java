...
public class SNACConstellationCreator {
    ...
    public Constellation createConstellationRecord(List<Row> rows) { // here
        ...
        if (temp_date_types.size() == temp_dates.size() && temp_date_types.size() != 0) {
            ...
            } else if (temp_date_types.size() == 2) { // this is under the assuption the rows are always in order of From and
     ...
     * If not: Create a new column "id" and insert cell values based on
     * constellation_ids
     */
